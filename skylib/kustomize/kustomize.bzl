...
#!/usr/bin/env bash
set -euo pipefail
{kustomize} build --load-restrictor LoadRestrictionsNone --reorder none {kustomize_dir} {template_part} {resolver_part} >{out}
"""
...
{}
    ...
    for inattr in ctx.attr.srcs:
        ...
        for infile in inattr.files.to_list():
            statements += ("echo $TARGET_DIR/{gitops_path}/{namespace}/{cluster}/{file}\n" +
                           ...
                           "{template_engine} --template={infile} --variable=NAMESPACE={namespace} --stamp_info_file={info_file} | {sorter} >> $TARGET_DIR/{gitops_path}/{namespace}/{cluster}/{file}\n").format(
                infile = infile.path,
                ...
                sorter = "${RUNFILES}/%s" % _get_runfile_path(ctx, ctx.executable._sorter),
            )
    ...
    runfiles = files + ctx.files.srcs + [ctx.executable._template_engine, ctx.file._info_file, ctx.executable._sorter]
...
def _kubectl_impl(ctx):
    ...
    delete = kubectl_command_arg == "delete"

    statements = ""
    transitive = None
    transitive_runfiles = []

    files += [ctx.executable._template_engine, ctx.executable._sorter, ctx.file._info_file]
    ...
    for inattr in ctx.attr.srcs:
        for infile in inattr.files.to_list():
            if not delete and ctx.attr.crds and len(ctx.attr.crds) > 0:
                statements += "{template_engine} --template={infile} --variable=NAMESPACE={namespace} --stamp_info_file={info_file} | {sorter} -filter=CustomResourceDefinition | kubectl --cluster=\"{cluster}\" --user=\"{user}\" {kubectl_command} -f -\n".format(
                ...
                statements += "kubectl wait --cluster=\"{cluster}\" --user=\"{user}\" --for condition=established --timeout=30s {crds}\n".format(
                    cluster = cluster_arg,
                    user = user_arg,
                    crds = " ".join(["crd/{crd}".format(crd=crd) for crd in ctx.attr.crds]),
                )
            statements += "{template_engine} --template={infile} --variable=NAMESPACE={namespace} --stamp_info_file={info_file} | {sorter} {reverse} | kubectl --cluster=\"{cluster}\" --user=\"{user}\" {kubectl_command} -f -\n".format(
                infile = infile.short_path,
                cluster = cluster_arg,
                user = user_arg,
                kubectl_command = kubectl_command_arg,
                ...
                info_file = ctx.file._info_file.short_path,
                sorter = "${RUNFILES}/%s" % _get_runfile_path(ctx, ctx.executable._sorter),
                reverse = "-reverse" if delete else "",
            )
